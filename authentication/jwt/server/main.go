...
func main() {
	...
	opts := []grpc.ServerOption{
		...
		grpc.UnaryInterceptor(ensureValidJWTCredentials),    // добавляем унарный перехватчик, для обработки аутентификации
	}
	...
	s := grpc.NewServer(opts...)                                        // создаем grpc-сервер c опциями
...
func valid(authorization []string) bool {
	...
	token, err := myJWT.VerifyToken(authorization[0])
	if err != nil {
		return false
	}
